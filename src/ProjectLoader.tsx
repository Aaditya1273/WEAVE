...
import { createWeaveStarterProject, STARTER_HERO_ID, STARTER_SITE_NAME } from './weave/starter-project';
import { armStarterFirstRun } from './weave/first-run';
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      if (fileCount > 0) {
        ...
        // WEAVE: a fresh STANDALONE session boots on the demo-ready ATELIER
        // starter (src/weave/starter-project.ts) instead of a blank page —
        // the human + agent demo needs a real site on the canvas from the
        // first second. Cloud keeps upstream's blank seed + template prompt.
        projectFS.loadSnapshot(CLOUD_ENABLED ? createEmptyProject() : createWeaveStarterProject());
        trace.action('project-loader:seeded-empty', { fileCount: projectFS.listFiles().length, starter: !CLOUD_ENABLED });
        // Fresh standalone session: name the project and arm the first-run
        // camera so the demo opens on a readable hero rather than a
        // fit-everything thumbnail. No-op for a saved project.
        if (!CLOUD_ENABLED) armStarterFirstRun(STARTER_HERO_ID, STARTER_SITE_NAME);
