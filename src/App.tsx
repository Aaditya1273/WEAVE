...
import { initWeave } from './weave/init';
import ProposalOverlay from './weave/ui/ProposalOverlay';
import InspectorOverlay from './weave/ui/InspectorOverlay';
...
export default function App() {
  ...
  // WEAVE — register the WebMCP tool surface + context subscription AFTER
  // mount, i.e. after ProjectLoader hydrated ProjectFS. Doing it at module
  // scope subscribed (and therefore computed) nodesAtom against the
  // module-default demo project before the real snapshot loaded; nothing
  // re-derives those atoms on hydration, so the canvas showed the demo while
  // ProjectFS held the loaded files (agent inserts then landed in the wrong
  // page, and reloads never showed saved work). Feature-detects the browser
  // API and degrades honestly.
  useEffect(() => { initWeave(); }, []);
  ...
  return (
    ...
    <div style={{ display: 'flex', height: '100vh', flexDirection: 'column' }}>
      ...
      {/* WEAVE — proposal review and the WebMCP inspector. Both self-gate on
          their own atoms, like the other portaled overlays here. */}
      <ProposalOverlay />
      <InspectorOverlay />
      <CommandPalette />
