...
// ─── WEAVE mark (WEAVE addition — replaces the upstream Revyme glyph) ─────
//
// A 24px ink tile with a woven "W". Fill tracks `--text-primary` via
// currentColor so it inverts with the builder theme like the glyph it
// replaced. Icon only — the logo slot is the 52px rail width, so a text
// wordmark clips; the product name lives in the tab title + Agent panel.

function WeaveLogo() {
  return (
    <span className="flex items-center" style={{ color: 'var(--text-primary)' }} title="WEAVE — Human + Agent Website Builder">
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width={24} height={24} aria-label="WEAVE">
        <rect x="1" y="1" width="22" height="22" rx="5" fill="currentColor" />
        <path d="M5 7l3 10 4-7 4 7 3-10" fill="none" stroke="var(--bg-surface, #fff)" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" />
      </svg>
    </span>
  );
}
