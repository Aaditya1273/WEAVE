...
// ─── Agent Icon (WEAVE addition) ────────────────────────────────────────────
//
// Two linked nodes — human + agent operating the same project. Opens the
// WEAVE Agent panel (WebMCP status, tool activity, publish approvals).
function AgentIcon({ className }: { className?: string }) {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" className={className}>
      <circle cx="6.5" cy="6.5" r="3.5" fill="currentColor" />
      <circle cx="17.5" cy="17.5" r="3.5" fill="currentColor" />
      <path stroke="currentColor" strokeWidth="2" strokeLinecap="round" d="M9 9l6 6" />
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center flex-col gap-2 relative z-10">
        ...
        {/* WEAVE Agent — the human + agent collaboration surface. First in
            the rail so the product's core promise is one click away.
            Enabled for viewers too — it is an observability surface; the
            tools themselves refuse writes in viewer mode at the mutation
            queue chokepoint. */}
        <MenuButton panelId="agent" isActive={activePanel === 'agent'} onToggle={togglePanel} title="WEAVE Agent" tooltip={tooltipHandlers} dataTutorial="agent-button">
          <AgentIcon className="w-[18px] h-[18px]" />
        </MenuButton>
