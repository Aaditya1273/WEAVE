...
//
// WEAVE addition: `VITE_SANDBOX_URL` overrides the derived port-based origin
// so a single-origin static deploy (Vercel) can serve the sandbox bundle from
// a path such as `/sandbox`. Process isolation via a separate origin is a
// performance nicety, not a functional requirement — the bridge is plain
// postMessage either way. Dev keeps the port-5174 default untouched.
export const SANDBOX_ORIGIN =
  (import.meta.env.VITE_SANDBOX_URL as string | undefined)?.replace(/\/$/, '') ||
