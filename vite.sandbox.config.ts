 ...
 */
/** WEAVE addition: public base path for this bundle. `/` (default) keeps the
 *  upstream port-per-bundle layout; `npm run build:vercel` sets it so the
 *  bundle can be served from a sub-path of ONE origin. */
const BASE = process.env.SANDBOX_BASE ?? '/';
...
function runtimeBridgeImportmapPlugin(): Plugin {
  return {
    ...
    transformIndexHtml(html) {
      // WEAVE: the importmap targets are absolute URLs, so a non-root `base`
      // (single-origin Vercel deploy, see SANDBOX_BASE) must be prefixed here too.
      return html.replace(/\/(runtime-bridge(?:-jsx|-motion)?)\.ts/g, `${BASE}$1.js`);
    },
  };
}
